...
%% Parameters algo
algo_options = struct('it',7,'it_end',2,'it_start',4);
...
radius = zeros(1,lin_index + algo_options.it + 1); 
radius(1:lin_index) = avg_distance(1:lin_index);
radius(lin_index + 1 : lin_index + algo_options.it) = linspace(avg_distance(lin_index+1), 1.1*r_max, algo_options.it);
radius(lin_index + algo_options.it + 1 ) = r_max_max;

radius = [ avg_distance(1:lin_index-2), ...
    linspace(avg_distance(lin_index-1),avg_distance(lin_index),algo_options.it_start), ...
    linspace(avg_distance(lin_index),r_max,algo_options.it), ...
    linspace(r_max,r_max_max,algo_options.it_end)] ;
...
hold on
pmax = plot([radius(lin_index), radius(lin_index)],ax.YLim, '-g');
hold on
scatter(radius,zeros(length(radius),1),'filled');
hold on
